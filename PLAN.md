...
- **실험 결과 (2026-03)**: .vstpreset 파일 생성 시도 완료. 파일 헤더/리스트 구조는 Cubase가 인식하지만 Comp(프로세서 상태) 바이너리 포맷이 비공개. 6가지 포맷 가설 테스트했으나 모두 실패. Cubase 9.5 StudioEQ는 프리셋을 내부 DB에 저장하여 파일 리버스 엔지니어링도 불가. → 보류
